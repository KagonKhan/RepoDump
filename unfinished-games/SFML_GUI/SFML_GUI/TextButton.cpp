...
TextButton::TextButton(opt_ref parent, const sf::Vector2f& size, const sf::Color& fillColor, std::string_view text, sf::Font& font, unsigned int charSize)
	: Button(parent), m_Shape(size), m_Label(opt_ref(*this), text, font, charSize), m_IsAutoAdjustable(false), m_Style(Style::Middle)
{
...
{
	m_IsAutoAdjustable = std::move(adjust);
}
